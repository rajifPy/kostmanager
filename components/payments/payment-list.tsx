...
export default function PaymentList({ payments }) {
  ...
  async function handleVerify(payment) {
    ...
    try {
      ...
      console.error('❌ Gagal verifikasi atau kirim notifikasi:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleReject(payment) {
    ...
    try {
      // Update status pembayaran ke 'rejected'
      const res = await fetch('/api/payments/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.log('✅ Notifikasi penolakan berhasil dikirim')
    } catch (error) {
      console.error('❌ Gagal tolak atau kirim notifikasi:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div>
      {payments.map((payment) => (
        <div key={payment.id} className="border p-4 mb-2">
          <p>Tenant ID: {payment.tenant_id}</p>
          <p>Status: {payment.status}</p>
          <button onClick={() => handleVerify(payment)} disabled={loading}>
            Verifikasi
          </button>
          <button onClick={() => handleReject(payment)} disabled={loading}>
            Tolak
          </button>
        </div>
      ))}
