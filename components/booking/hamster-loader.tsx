...
export function HamsterLoader({ size = "md" }: { size?: "sm" | "md" | "lg" }) {
  const [isVisible, setIsVisible] = useState(false);

  // Prevent hydration mismatch by rendering loader only on client
  useEffect(() => {
    setIsVisible(true);
  }, []);
  ...
  return (
    <div 
      className={`${scaleMap[size]} inline-block`}
      aria-label="Hamster berlari di roda — memuat..."
      role="status"
    >
      <div className="wheel-and-hamster" style={{ fontSize: "14px", width: "12em", height: "12em" }}>
        <div className="wheel" />
